...
bool FreeImageB::LoadImageFromFile(const std::string& filename, unsigned int width, unsigned int height)
{
	...
	// try to convert FreeImage to OpenCV, failed
	
	//int cv_type = cv::DataType<cv::Vec<float, 4>>::generic_type;
	//int cv_cvt = cv::COLOR_RGBA2BGRA;
	//int step = FreeImage_GetPitch(dib);
	//cv::Mat img(h, w, cv_type, FreeImage_GetBits(dib), step);
	//cv::cvtColor(img, img, cv_cvt);
	//cv::imshow("Test", img);
	//cv::waitKey(0);
