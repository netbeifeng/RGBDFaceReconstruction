...
public:
	Matrix4f estimatePose(const std::vector<Vector3f>& sourcePoints, const std::vector<Vector3f>& targetPoints) {
		...
		auto scaleFactor = estimateScale(sourcePoints, sourceMean, targetPoints, targetMean);
		std::cout << "Estimated scale " << scaleFactor << std::endl;

		std::vector<Vector3f> sourcePoints_scaled;
		const unsigned numPoints = sourcePoints.size();
		for (int i = 0; i < numPoints; i++)
			sourcePoints_scaled.push_back(scaleFactor * sourcePoints[i]);
		
		Matrix3f rotation = estimateRotation(sourcePoints_scaled, sourceMean * scaleFactor, targetPoints, targetMean);
...
private:
	...
	float estimateScale(const std::vector<Vector3f>& sourcePoints, const Vector3f& sourceMean, const std::vector<Vector3f>& targetPoints, const Vector3f& targetMean) {
		// Estimate the scale
		const unsigned nPoints = sourcePoints.size();
		...
		for (int i = 0; i < nPoints; ++i) {
			sourceVariance += (sourcePoints[i] - sourceMean).squaredNorm();
			targetVariance += (targetPoints[i] - targetMean).squaredNorm();
		}

		// since both points have the same size, the formula becomes a bit shorter
		double scaleFactor = sqrt(targetVariance) / sqrt(sourceVariance);

		
		std::cout << sourceVariance << ", " << targetVariance << ", " << scaleFactor << std::endl;
