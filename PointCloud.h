...
#pragma omp parallel for
    ...
    void static writeVerticesToPcd(std::string filename, Vertex* vertices, int length) {

        std::vector<std::pair<int, Vertex>> outputVertices;
        ...
        // screen out those points are out of depth
        for (int i = 0; i < length; i++) {
            Vertex ithVertex = vertices[i];
            if (ithVertex.position.z() < 0.15 && ithVertex.position.z() != MINF) {
                outputVertices.push_back(std::make_pair(i, ithVertex));
                ssPoints << ithVertex.position.x() << " " << ithVertex.position.y() << " " << ithVertex.position.z() << std::endl;
            }
        }

        std::cout << outputVertices.size() << std::endl;
        std::ofstream outFile(filename + ".pcd");
        if (!outFile.is_open()) {
            throw std::runtime_error("Not open");
        }
        outFile << "# .PCD v.7 - Point Cloud Data file format" << std::endl; // header
        outFile << "VERSION .7" << std::endl; // header
        outFile << "FIELDS x y z" << std::endl; // header
        outFile << "SIZE 4 4 4" << std::endl; // header
        outFile << "TYPE F F F" << std::endl; // header
        outFile << "COUNT 1 1 1" << std::endl; // header
        outFile << "WIDTH " << outputVertices.size() << std::endl; // header
        outFile << "HEIGHT 1" << std::endl; // header
        outFile << "VIEWPOINT 0 0 0 1 0 0 0" << std::endl; // header
        outFile << "POINTS " << outputVertices.size() << std::endl; // header
        outFile << "DATA ascii" << std::endl; // header
        outFile << ssPoints.rdbuf();
        outFile.close();
    }
